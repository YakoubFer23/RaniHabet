...
@endif
                <main>
                    <div class="container-fluid px-4">
                        <div style="display: flex; align-items: center;">
                            <h1 class="mt-4" style="flex: 3;">Properties</h1>
  ...
  @foreach ( $pendingListings as $dar )
    <tr>
      
      <td>{{$dar->title}}</td>
      <td>{{$dar->user->firstname}} {{$dar->user->lastname}} </td>
      <td>
        <form action="{{route('listings.show', ['id' => $dar->id])}}" target="_blank" method="get">
      ...
      <td>
        <form action="{{route('tejwak.diour')}}" method="POST">
            @csrf
            <input type="hidden" name="approve" value="{{$dar->id}}">
            <button type="submit" class="btn btn-primary" style="background-color: green;">Approve</button></td>
        </form>
